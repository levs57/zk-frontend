use std::{cmp::max, collections::{HashSet, VecDeque}, marker::PhantomData, sync::Arc};
...
impl<C : Circuit + StandardVariables + 'static> CommitmentGroup<C> for SimplePubCommGroup<C> {
    ...
    fn new(circuit: &mut C, _: &()) -> Self {
        Self{signals: HashSet::new()}
    }
    ...
    fn commit(self, circuit: &mut C) -> Self::CommitmentObject {
        ...
        Box::new(move |circuit: &mut C| {
            let ret : Vec<_> = (*sigvec).into_iter().map(|sig|{circuit.read_sig(*sig)})
                .collect();
            ret
        })
    }
...
impl<C : Circuit + StandardVariables, T: TypeConfig<F = C::F>> CommitmentGroup<C> for SimplePrivCommGroup<C, T> {
    type CommitmentObject = Box<dyn Fn(&mut C) -> T::G>;
    type Params = Arc<Vec<T::G>>;


    fn new(circuit: &mut C, params: &Self::Params) -> Self {
        Self{signals: HashSet::new(), params: params.clone()}
    }
...
impl<T: TypeConfig> CircuitGenericAdvices for SimpleCircuit<T> {
    ...
    > (&mut self, f: Fun, num_outputs: usize, inp: Vec<I>) -> Vec<O> {
        ...
        let op = Box::new(move |c: &mut Self| {
            ...
            output.clone().iter().zip(out.into_iter()).map(|(target, value)|{target.write_to(c, value)}).count();
        });
        self.operations.push_back(Operation::Internal(op));
        ret    
    }
...
impl<T: TypeConfig> CircuitGenericConstraints for SimpleCircuit<T> {
    fn enforce<
        ...
        Fun: Fn(I::FStruct) -> O::FStruct + 'static
    >(&mut self, f: Fun, deg: usize, inp: I) -> O {
        let output = O::alloc_to(self);

        inp.to_raw_addr(&self).into_iter().map(|raw|{
        ...
        let constr = ConstrEncoding {constr, deg, _marker : PhantomData};
    ...
    > (&mut self, f: Fun, deg: usize, num_outputs: usize, inp: Vec<I>) -> Vec<O> {
        ...
        let raws : Vec<_> = inp.iter().map(|st|st.to_raw_addr(&self)).flatten().map(|raw|{
            <Self as HasSigAddr<_>>::try_parse_raw_addr(&self, raw)}
            ).collect();
            
        raws.into_iter().map(|idx|{
            self.signals[idx].1 = max(self.signals[idx].1, deg);
        }).count(); // Iterate through addresses and increase metadata parameter degree.
